...
const Exporter = {
     ...
     */
    countColors: (gridResponse) => {
        const counts = {};
        const totalPixels = gridResponse.width * gridResponse.height; // Note: this includes empty pixels if we don't subtract
        let validPixels = 0;

        // Flatten grid
        gridResponse.grid.forEach(row => {
            row.forEach(cell => {
     ...
     */
    generatePrintableCanvas: (gridResponse, opts = {}) => {
        const cellSize = opts.cellSize || 20; // larger size for readability
        const width = gridResponse.width * cellSize;
        const height = gridResponse.height * cellSize;
        ...
        // Draw cells
        gridResponse.grid.forEach((row, y) => {
            row.forEach((cell, x) => {
